...
def print_county_report(report: dict):
    ...
    # ---- nearest_any table ----
    st.subheader("5 Closest Counties by Projected 2030 Birth Rate")

    df_any = report["nearest_any"].copy()
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "rate": st.column_config.NumberColumn("2030 Birth Rate (per 1,000)", format="%.2f", width="small"),
            "is_selected": None,
        },
    )

    # ---- nearest_same_regime table ----
    st.subheader("5 Closest Counties in the Same Socio-Political Category")
