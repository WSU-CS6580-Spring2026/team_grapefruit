...
st.markdown(
    """
    <h1 style="color: #279a9f; margin-bottom: 0;">
        US County Birthrate<br>
        Projection Tool
    </h1>
    """,
...
st.markdown(
    """
    <div style="color: #b0c4de; font-style: italic;">
    <ul>
        <li>Explore historical and projected birth rates across the most populous U.S. counties.</li>
        <li>Compare trends from 2010 to 2020 and projections to 2030</li>
        <li>Identify counties facing similar demographic challenges</li>
        <li>Discover potential patterns and insights across socio-political groups</li>
        <li>Use the state map to visually compare projected birth rates within each state</li>
    </ul>
    </div>
    """,
    unsafe_allow_html=True
...
df_app = (
    ...
    .merge(df_counties, on="fips", how="left")
)
...
# Filter options
# -------------------------
state_options = ["All"] + sorted(df_app["state"].dropna().unique().tolist())
category_options = ["All"] + sorted(df_app["regime_2020"].dropna().unique().tolist())
...
selected_category = st.sidebar.selectbox(
    "Filter by Socio-Political Category",
    category_options,
    index=0
)
...
if selected_category != "All":
    df_filtered = df_filtered[df_filtered["regime_2020"] == selected_category]

# Build county list from filtered dataframe
county_list = sorted(df_filtered["dropdown_label"].dropna().tolist())

# Fallback in case filters produce no counties
if not county_list:
    st.sidebar.warning("No counties match the selected filters.")
    county_list = sorted(df_app["dropdown_label"].dropna().tolist())

# Default selection logic
default_county = "Utah - Weber"
default_index = county_list.index(default_county) if default_county in county_list else 0
